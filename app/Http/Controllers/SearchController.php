...
//  ]);
 ...
 //$texts =  DB::table('arabics')->select('*')

$texts =  DB::select('select * from arabics  ORDER BY RAND() LIMIT 1;');
 //->join('arabics','arabics.datasurah_id', '=', 'datasurahs.id')
 //->join('thais', 'thais.id', '=', 'arabics.id')
  ...
  return view('random',['texts'=>$texts]);
  //return response()->view('your_view', compact('variableName'));
//  return response()->view('search', compact('texts'));
   //return response()->json($texts);
...
function random_enduser(Request $request){
  //   $request->validate([
  //     'query'=>'required|min:1'
  //  ]);
  ...
  $texts =  DB::select('select * from arabics  ORDER BY RAND() LIMIT 1;');
   ...
   //->join('thais', 'thais.id', '=', 'arabics.id')
    ...
    return view('random_enduser',['texts'=>$texts]);
    //return response()->view('your_view', compact('variableName'));
  //  return response()->view('search', compact('texts'));
     //return response()->json($texts);
